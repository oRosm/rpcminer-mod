...
OpenCLRunner::OpenCLRunner():GPURunner<cl_uint,cl_uint>(TYPE_OPENCL),m_platform(0)
{
	...
	std::string opencl_module_path("bitcoinmineropencl.cl");
	...
	if(numplatforms>0 && m_platform>=0 && m_platform<numplatforms)
	{
		...
		if(m_devicecount>0)
		{
			...
			std::string srcfile=ReadFileContents(opencl_module_path);
	...
	for(int numb=lowb; numb<=highb; numb*=2)
	{
		for(int numt=lowt; numt<=hight; numt*=2)
		{
			...
			for(int it=0; it<128*256*mult && err==CL_SUCCESS; it+=(numb*numt))
			{
				...
				const size_t cnumt=numt;
				const size_t dim=numt*numb;
...
const cl_uint OpenCLRunner::RunStep(uint32 nonce)
{
	...
	*/
	...
	const size_t cnumt=m_numt;
	const size_t dim=m_numt*m_numb;
...
const std::string OpenCLRunner::ReadFileContents(const std::string &filename) const
{
	...
	if(infile)
	{
		...
		if(len>0)
		{
			...
			size_t read_size = fread(&data[0],1,data.size(),infile);

			if (read_size != len)
			{
				printf("Error while reading file: %s\nFile size: %u\nRead size: %u",
					   filename.c_str(), len, read_size);
			}
		}
