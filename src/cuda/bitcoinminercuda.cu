...
#endif
{
    ...
    const unsigned int nonce = nonceIn + (myid << bits);
    unsigned int t1, t2;
    ...
    unsigned int in_m_merkle = in[cudaIn::merkle];
    unsigned int in_m_ntime = in[cudaIn::ntime];
    unsigned int in_m_nbits = in[cudaIn::nbits];
    ...
    if (threadIdx.x < 8)
    {
        AH2[threadIdx.x] = AH[threadIdx.x] = in[threadIdx.x];
    }
    __syncthreads();
    if (threadIdx.x == 0)
    {
